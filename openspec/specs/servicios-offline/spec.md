...
This change requires NO backend modifications. Both `/servicios/partos` and `/servicios/palpaciones` routes already have `idempotencyMiddleware` active, which handles deduplication on replay.