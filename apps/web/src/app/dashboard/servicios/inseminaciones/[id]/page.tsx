// apps/web/src/app/dashboard/servicios/inseminaciones/[id]/page.tsx
/**
 * Inseminación detail page — detalle de evento grupal con resultados por animal.
 */
...
import { useInseminacion } from '@/modules/servicios';
import { Button } from '@/shared/components/ui/button';

export default function InseminacionDetailPage(): JSX.Element {
  const params = useParams();
  const id = Number(params.id);
  if (isNaN(id)) return notFound();
  const { data, isLoading, error } = useInseminacion(id);
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <Link href="/dashboard/servicios/inseminaciones">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Detalle del evento de inseminación
          </p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 text-left text-xs font-medium text-gray-500 dark:border-gray-700 dark:text-gray-400">
                ...
                <th className="px-6 py-3">Toro</th>
                <th className="px-6 py-3">Pajuela</th>
              ...
              {data.animales.map((animal) => (
                <tr key={animal.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300">
                    {new Date(animal.fecha).toLocaleDateString('es-CO')}
                  </td>
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300">
                    {animal.toro ?? '-'}
                  </td>
