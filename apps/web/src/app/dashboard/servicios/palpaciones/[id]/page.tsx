// apps/web/src/app/dashboard/servicios/palpaciones/[id]/page.tsx
/**
 * Palpación detail page — detalle de evento grupal con resultados por animal.
 */
...
import { usePalpacion } from '@/modules/servicios';
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <Link href="/dashboard/servicios/palpaciones">
          <Button variant="secondary" className="mt-4">
            <ArrowLeft className="mr-2 h-4 w-4" />
            Volver a palpaciones
          </Button>
        </Link>
      </div>
    );
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Detalle del evento de palpación
          </p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 text-left text-xs font-medium text-gray-500 dark:border-gray-700 dark:text-gray-400">
                ...
                <th className="px-6 py-3">Condición Corp.</th>
                <th className="px-6 py-3">Días Gestación</th>
                <th className="px-6 py-3">Fecha Parto</th>
              ...
              {data.animales.map((animal) => (
                <tr key={animal.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                        animal.diagnosticoNombre === 'Positiva'
                  ...
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300">
                    {animal.condicionCorporalNombre ?? '-'}
                  </td>
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300">
                  ...
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300">
                    {animal.fechaParto
                      ? new Date(animal.fechaParto).toLocaleDateString('es-CO')
                      : '-'}
                  </td>
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300">
                    {animal.comentarios ?? '-'}
