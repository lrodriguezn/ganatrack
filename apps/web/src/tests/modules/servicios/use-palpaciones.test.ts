// apps/web/src/tests/modules/servicios/use-palpaciones.test.ts
/**
 * usePalpaciones hook tests.
 *
 * Tests:
...
// Import AFTER mock
const { usePalpaciones } = await import('@/modules/servicios/hooks/use-palpaciones');
...
describe('usePalpaciones', () => {
  describe('basic usage', () => {
    it('debería retornar data y estados de carga', async () => {
      const mockData = {
        data: [
          { id: 1, animalId: 1, fecha: '2024-01-15', resultado: 'positiva' },
        ],
        page: 1,
        limit: 10,
        total: 1,
      ...
      mockServiciosService.getPalpaciones.mockResolvedValueOnce(mockData);

      const { result } = renderHook(
        () => usePalpaciones({ page: 1, limit: 10 }),
    ...
    it('debería retornar error cuando el servicio falla', async () => {
      mockServiciosService.getPalpaciones.mockRejectedValueOnce(new Error('API Error'));
