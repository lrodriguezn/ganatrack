// apps/web/src/tests/modules/servicios/use-inseminaciones.test.ts
/**
 * useInseminaciones hook tests.
 *
 * Tests:
...
// Import AFTER mock
const { useInseminaciones } = await import('@/modules/servicios/hooks/use-inseminaciones');
...
describe('useInseminaciones', () => {
  describe('basic usage', () => {
    it('debería retornar data y estados de carga', async () => {
      const mockData = {
        data: [
          { id: 1, animalId: 1, fecha: '2024-01-15', resultado: 'pendiente' },
        ],
        page: 1,
        limit: 10,
        total: 1,
      ...
      mockServiciosService.getInseminaciones.mockResolvedValueOnce(mockData);

      const { result } = renderHook(
        () => useInseminaciones({ page: 1, limit: 10 }),
    ...
    it('debería retornar error cuando el servicio falla', async () => {
      mockServiciosService.getInseminaciones.mockRejectedValueOnce(new Error('API Error'));
