// apps/web/src/tests/mocks/handlers/productos.handlers.ts
/**
...
export const productosHandlers = [
  ...
  http.get(`${BASE_URL}/api/v1/productos`, ({ request }) => {
  ...
  http.get(`${BASE_URL}/api/v1/productos/:id`, ({ params }) => {
  ...
  http.post(`${BASE_URL}/api/v1/productos`, async ({ request }) => {
  ...
  http.put(`${BASE_URL}/api/v1/productos/:id`, async ({ params, request }) => {
    const id = Number(params.id);
  ...
  http.delete(`${BASE_URL}/api/v1/productos/:id`, ({ params }) => {
    const id = Number(params.id);
    const index = mockProductos.findIndex(p => p.id === id);
