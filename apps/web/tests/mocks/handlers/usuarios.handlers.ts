// apps/web/tests/mocks/handlers/usuarios.handlers.ts
/**
...
export const usuariosHandlers = [
  ...
  http.get('*/api/v1/usuarios/:id', ({ params }) => {
  ...
  http.put('*/api/v1/usuarios/:id', async ({ params, request }) => {
    const id = Number(params.id);
  ...
  http.patch('*/api/v1/usuarios/:id/activate', ({ params }) => {
    const id = Number(params.id);
    const index = mockUsuarios.findIndex(u => u.id === id);
