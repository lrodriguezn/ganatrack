...
import type { ImagenEntity } from '../../domain/entities/animal.entity.js'
...
export class DrizzleImagenRepository implements IImagenRepository {
  ...
  async softDelete(id: number, predioId: number): Promise<boolean> {
    await this.db.update(imagenes).set({ activo: 0, updatedAt: new Date() }).where(and(eq(imagenes.id, id), eq(imagenes.predioId, predioId)))
    return true
  }
}
